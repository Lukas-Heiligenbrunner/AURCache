...
pub fn get_build_mode() -> BuildMode {
    match env::var("BUILD_ARTIFACT_DIR") {
        Ok(v) => {
            ...
            let mirrorlist_path_aurcache = format!("{}/config/mirrorlist_x86_64", build_artifact_dir_aurcache);
            ...
            let mirrorlist_path_host = match env::var("MIRRORLIST_PATH_X86_64") {
                Ok(v) => v,
                Err(_) => format!("{}/config/mirrorlist_x86_64", v),
            };
            
            // create config dir if not existing
            create_config_dir(format!("{}/config", build_artifact_dir_aurcache));
        ...
        Err(_) => {
            let mirrorlist_path = match env::var("MIRRORLIST_PATH_X86_64") {
                ...
                Err(_) => {
                    ...
                    // create config dir if not existing
                    create_config_dir(format!("{}/config", config_dir.display()));
